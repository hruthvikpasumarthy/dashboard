...
if(x[j].status=="Abandoned")
{
  ...
  table += '<p style="margin : 0;padding-left: 5px;float: left;"><sasync function add(items){
    $(document).ready(function () {
var table = '<div class="grid-container">';
for (var i = 0; i < items.length; i++) {
table += '<div class="grid-item"><center> <p style="color:black;font-size:20px;padding-left: 5px;"><strong>';
...
$('div').html(table);
    ...
    async function callAPI(asset,service,type){
        ...
        if(document.getElementById('asset').value=="" && document.getElementById('service').value=="" && document.getElementById('type').value=="All" ){
            let response = await fetch("https://{{gatewayid}}.execute-api.us-east-1.amazonaws.com/dev/{{apiresourcename}}?type="+"All");
           ...
           add(item)
            ...
            let response = await fetch("https://{{gatewayid}}.execute-api.us-east-1.amazonaws.com/dev/{{apiresourcename}}?asset="+asset+"&type="+type);
            let result= await response.text();
            let items= await JSON.parse(result).body;
            let item=JSON.parse(items);
            if (item['msg']=="No pipelines found for these filters")
           ...
           add(item)
            ...
            let response = await fetch("https://{{gatewayid}}.execute-api.us-east-1.amazonaws.com/dev/{{apiresourcename}}?service="+service+"&type="+type);
            let result= await response.text();
            let items= await JSON.parse(result).body;
            let item=JSON.parse(items);
            if (item['msg']=="No pipelines found for these filters")
           ...
           add(item)
            ...
            let response = await fetch("https://{{gatewayid}}.execute-api.us-east-1.amazonaws.com/dev/{{apiresourcename}}?type="+type);
            let result= await response.text();
            let items= await JSON.parse(result).body;
            let item=JSON.parse(items);
            if (item['msg']=="No pipelines found for these filters")
           ...
           add(item)
            ...
            let response = await fetch("https://{{gatewayid}}.execute-api.us-east-1.amazonaws.com/dev/{{apiresourcename}}?asset="+asset+"&service="+service+"&type="+type);
            let result= await response.text();
            let items= await JSON.parse(result).body;
            let item=JSON.parse(items);
            if (item['msg']=="No pipelines found for these filters")
